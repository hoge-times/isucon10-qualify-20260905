...
- **ログは既定で切ってある。** nginx のアクセスログ / MySQL のスロークエリログ / アプリのアクセスログは `make logoff` の状態が既定で、スコアを狙う走行はこれで回す(詳細は [report/disable-logs.md](report/disable-logs.md))。

  | 用途 | 手順 |
  |---|---|
  | 計測(alp / pt を採る) | `make prepare` または `report.sh` — **自動でログが ON に戻る** |
  | 本番走行(スコア狙い) | `make logoff` → ベンチ。**`prepare` を挟むと ON に戻るので注意** |
  | 手で戻す | `make logon` |
...
| 2026-09-05 18:03 | 検索のバケット化 (#34) を取り込んで再計測(score 17342)。**ボトルネックは web (i1 93.6%) に移り**、DB は i2 110.2% / i3 58.7% に下がった | iwashi623 |
| 2026-09-05 18:30 | nginx / MySQL / アプリのログを既定で停止(`make logon` / `logoff` で切り替え)。score 17204 → 22969 | iwashi623 |
